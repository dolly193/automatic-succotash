...
const initializeEfiPay = () => {
  ...
  let certPath = process.env.EFI_CERTIFICATE_PATH;
  const certBase64 = process.env.EFI_CERTIFICATE_BASE64;

  // Fallback para o caminho padrão do Render em produção, caso a variável de ambiente não seja detectada a tempo.
  if (isProduction && !certPath && !certBase64) {
    // IMPORTANTE: Substitua 'producao.p12' pelo nome exato do seu arquivo de certificado no Render.
    const renderSecretPath = '/etc/secrets/EFI_CERTIFICATE_PATH'; 
    console.log(`[Efí Pay Service] Variável de ambiente não encontrada. Tentando caminho padrão do Render: ${renderSecretPath}`);
    certPath = renderSecretPath;
  }

  if (certPath) { // A lógica agora usará o caminho do Render se a variável de ambiente falhar.
